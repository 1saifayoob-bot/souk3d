...
export function rowToProduct(r) {
  ...
  return {
    ...
    // True while a product is still inside its members-only early access window.
    earlyAccess: !!(r.public_at && new Date(r.public_at).getTime() > Date.now()),
    images,
...
export function productToRow(p) {
  ...
  return {
    ...
    public_at: p.publicAt ? p.publicAt : null,
    images,
    description: p.desc || "",
