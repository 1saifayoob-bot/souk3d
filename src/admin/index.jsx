            ...
            <div>
              ...
              <section id="pf-pricing" className="s3d-sec">
                ...
                <div style={{ marginTop: 22 }}>
                  ...
                  <div style={{ ...hint, marginTop: 0, marginBottom: 10 }}>The price change is added to the base price: 2 means +$2.00, -1.5 means $1.50 less, empty means same price. Pick a photo for a choice and the store shows that photo when a customer selects it — and a shared link points straight at it. For colours, the dot on the product card is guessed from the name (Pink, Blue, Gold…); use the colour box to set it exactly.</div>
                    ...
                    <div key={gi} style={{ border: "1px solid " + COLORS.wheat, borderRadius: 10, padding: 12, marginBottom: 10, background: COLORS.cream }}>
                      ...
                      {(g.options || []).map(function (o, oi) { return (
                        <div key={oi} style={{ display: "flex", gap: 8, marginBottom: 6, alignItems: "center" }}>
                          ...
                          <input type="color" value={o.color || "#D4881F"} onChange={function (e) { var v = (form.variations || []).slice(); var opts = (v[gi].options || []).slice(); opts[oi] = { ...opts[oi], color: e.target.value }; v[gi] = { ...v[gi], options: opts }; set("variations", v); }} aria-label="Swatch colour" title={o.color ? "Swatch colour " + o.color : "Set the swatch colour shown on the product card"} style={{ width: 38, height: 38, padding: 2, border: "1px solid " + COLORS.wheat, borderRadius: 8, background: "#fff", cursor: "pointer", opacity: o.color ? 1 : 0.55 }} />
                          {o.color ? <button onClick={function () { var v = (form.variations || []).slice(); var opts = (v[gi].options || []).slice(); opts[oi] = { ...opts[oi], color: null }; v[gi] = { ...v[gi], options: opts }; set("variations", v); }} className="s3d-btn-small" title="Use the colour guessed from the name">auto</button> : null}
                          <select value={o.img === 0 || o.img ? String(o.img) : ""} onChange={function (e) { var v = (form.variations || []).slice(); var opts = (v[gi].options || []).slice(); opts[oi] = { ...opts[oi], img: e.target.value === "" ? null : parseInt(e.target.value, 10) }; v[gi] = { ...v[gi], options: opts }; set("variations", v); }} aria-label="Photo for this choice" title="Show this photo when the choice is picked" style={{ ...inputStyle(false), flex: 1, cursor: "pointer" }}>
                            <option value="">Any photo</option>
                            {(form.images || []).map(function (im, k) { return <option key={k} value={k}>Photo {k + 1}{k === 0 ? " (cover)" : ""}</option>; })}
                          </select>
